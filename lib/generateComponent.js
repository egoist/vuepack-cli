...
mkdirp(path.join(process.cwd(), 'src/components'))
mkdirp(path.join(process.cwd(), 'src/css'))
...
module.exports = function () {
  ...
  if (!componentName) {
    ...
    if ((!argv.f || !argv.force) && (cssPathExists || componentPathExists)) {
      if (cssPathExists) {
        log.error(`${'CSS file'.white.bold} of the ${'Component'.white.bold} exists, use ${'-f/--force'.white.bold} to force update!`)
      }
      if (componentPathExists) {
        log.error(`The ${'Component'.white.bold} exists, use ${'-f/--force'.white.bold} to force update!`)
      }
    } else {
      ...
      log.success(`Component ${componentName.white.bold} is created!`)
    }
  }
}
